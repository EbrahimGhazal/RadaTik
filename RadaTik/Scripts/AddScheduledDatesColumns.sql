...
-- Use when EF reports Duplicate column name 'ScheduledInstallationDate' / 'ScheduledVisitDate'

IF OBJECT_ID(N'dbo.Clients', N'U') IS NOT NULL
   AND COL_LENGTH(N'dbo.Clients', N'ScheduledInstallationDate') IS NULL
BEGIN
    ALTER TABLE [dbo].[Clients] ADD [ScheduledInstallationDate] datetime2 NULL;
    PRINT N'Added Clients.ScheduledInstallationDate';
END
ELSE
    PRINT N'Skipped Clients.ScheduledInstallationDate (table missing or column exists)';

IF OBJECT_ID(N'dbo.MaintenanceRequests', N'U') IS NOT NULL
   AND COL_LENGTH(N'dbo.MaintenanceRequests', N'ScheduledVisitDate') IS NULL
BEGIN
...
END
ELSE
    PRINT N'Skipped MaintenanceRequests.ScheduledVisitDate (table missing or column exists)';
