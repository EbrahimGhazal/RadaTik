...
{
    Task NotifyMaintenanceRequestSubmittedAsync(
        MaintenanceRequest request,
        int? networkId,
        string? clientName,
        string? clientUserName);

    Task NotifySpeedChangeRequestSubmittedAsync(
    ...
    Task NotifyClientJoinRequestSubmittedAsync(JoinRequest request);
    Task NotifyEmployeeJoinRequestSubmittedAsync(JoinRequest request);
    Task NotifyJoinRequestSubmittedAsync(JoinRequest request);

    Task NotifyClientTopUpSubmittedAsync(
        int clientId,
        int? networkId,
        decimal amount,
    ...
    Task NotifyCollectionPointTopUpRequestSubmittedAsync(
        CollectionPointTopUpRequest request,
        string? requesterDisplayName);

    Task NotifyClientWalletTopUpRequestSubmittedAsync(
        ClientWalletTopUpRequest request,
        string? clientDisplayName);

    Task NotifyMaintenanceInvoiceIssuedAsync(
        MaintenanceInvoice invoice,
        string? clientUserName,
        string? clientName);

    Task NotifyMaintenanceInvoicePaidAsync(MaintenanceInvoice invoice);
}
...
public class RequestNotificationService : IRequestNotificationService
{
    ...
    /// <summary>
    /// إشعار مدير النظام بطلب انضمام (مدير شركة أو نقطة تحصيل).
    /// </summary>
    public async Task NotifyJoinRequestSubmittedAsync(JoinRequest request)
    {
        ...
        string typeLabel = request.RequestType == JoinRequestType.CollectionPoint
            ? "نقطة تحصيل"
            : "مدير شركة";
        string title = "طلب انضمام جديد";
        string message = $"تم تقديم طلب انضمام ({typeLabel}) من {request.FullName} ({request.Email}).";

        await CreateNotificationsAsync(
            recipients,
            NotificationType.JoinRequestSubmitted,
            title,
            message,
            $"JoinRequestSubmitted:{request.Id}",
            null);
    }
        ...
        string? requesterDisplayName)
    {
        ...
        int? networkId = request.CollectionPointAccount?.NetworkId;
        ...
        string? clientName)
    {
        ...
        string title = "فاتورة صيانة جديدة";
        string message =
    ...
    {
        List<string> recipients = await GetCompanyAdminRecipientsAsync(invoice.NetworkId);
        if (recipients.Count == 0)
        {
            return;
        }

        string title = "تم سداد فاتورة صيانة";
        string message =
