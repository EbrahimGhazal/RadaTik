...
internal static class StandalonePricingRowMutator
{
    ...
    {
        foreach (FeaturePricing row in rows)
        {
            row.IsActive = false;
            row.UpdatedAt = now;
        }
...
internal static class StandalonePricingRecordStore
{
    ...
    {
        IQueryable<FeaturePricing> query = db.FeaturePricings.Where(p =>
        ...
        FeaturePricing? pricing = await query.FirstOrDefaultAsync(ct);
        if (pricing != null)
        {
            return pricing;
        }
...
public sealed class ReportPricingHandler : IReportPricingHandler
{
    ...
    public async Task<StandalonePricingUpdateResult> UpdateAsync(decimal initialPriceSyp, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            FeaturePricing reportPricing = await StandalonePricingRecordStore.GetOrCreateOneTimeAsync(
                _db,
            ...
            List<FeaturePricing> duplicateRows = await _db.FeaturePricings
                .Where(p =>
...
public sealed class ProfileTaxPricingHandler : IProfileTaxPricingHandler
{
    ...
    public async Task<StandalonePricingUpdateResult> UpdateAsync(decimal taxPercentage, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            FeaturePricing profileTaxPricing = await StandalonePricingRecordStore.GetOrCreateOneTimeAsync(
                _db,
...
public sealed class MaintenanceCommissionPricingHandler : IMaintenanceCommissionPricingHandler
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            DateTime now = DateTime.UtcNow;
            PricingChargeUnit chargeUnit = commissionMode == MaintenanceCommissionMode.Percent
                ? PricingChargeUnit.PercentOfCollectedAmount
                : PricingChargeUnit.Flat;

            FeaturePricing maintenanceCommission = await StandalonePricingRecordStore.GetOrCreateOneTimeAsync(
                _db,
