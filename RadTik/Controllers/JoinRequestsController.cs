    ...
    public class JoinRequestsController : Controller
    {
        ...
        private async Task ApproveNetworkAdminRequest(JoinRequest request, ApplicationUser? currentUser, string? adminNotesFromForm)
        {
            try
            {
                ...
                {
                    var notesParts = request.Notes.Split(new[] { "اسم المستخدم المطلوب: " }, StringSplitOptions.None);
                ...
                {
                    var adminNotesParts = adminNotesFromForm.Split(new[] { "كلمة المرور المطلوبة: " }, StringSplitOptions.None);
