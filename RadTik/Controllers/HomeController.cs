...
namespace RadTik.Controllers
{
    public class HomeController : Controller
    {
        ...
        private async Task<IActionResult> SystemDashboard()
        {
            ...
            var totalCompanies = await _context.Networks.CountAsync(n => n.ParentNetworkId == null);
            var totalSubNetworks = await _context.Networks.CountAsync(n => n.ParentNetworkId != null);

            // إحصائيات عامة على كامل النظام
            var totalClients = await _context.Clients.CountAsync();
            var activeClients = await _context.Clients.CountAsync(c => c.IsActive);
            var totalServers = await _context.MikroTikServers.CountAsync();
            var totalSectors = await _context.Sectors.CountAsync();
            var totalReceivers = await _context.Receivers.CountAsync();

            // طلبات مديري الشركة
            var pendingCompanyManagers = await _context.JoinRequests.CountAsync(r =>
        ...
        private async Task<IActionResult> EmployeeDashboard()
        {
            ...
            var expiringClients = await _context.Clients
        ...
        private async Task<IActionResult> ClientDashboard(ApplicationUser user)
        {
            ...
            {
                var remaining = client.AccountExpirationDate.Value - DateTime.Now;
            ...
            {
                var totalDays = (client.AccountExpirationDate.Value - client.CreatedDate).TotalDays;
                var usedDays = (DateTime.Now - client.CreatedDate).TotalDays;
