...
public sealed class MikroTikSyncBackgroundService : BackgroundService
{
    ...
    private async Task ProcessJobAsync(MikroTikSyncJob job, CancellationToken cancellationToken)
    {
        ...
        var mikroTikUsersService = scope.ServiceProvider.GetRequiredService<IMikroTikUsersService>();
        var mikroTikProfilesService = scope.ServiceProvider.GetRequiredService<IMikroTikProfilesService>();
        var context = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();

        try
        {
            if (job.EntityType == nameof(Client))
                await ProcessClientJobAsync(mikroTikUsersService, context, job, cancellationToken);
    ...
    private async Task ProcessClientJobAsync(IMikroTikUsersService mikroTikService, ApplicationDbContext context, MikroTikSyncJob job, CancellationToken cancellationToken)
    {
