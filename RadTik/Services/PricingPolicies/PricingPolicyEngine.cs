...
public sealed class FeaturePublicContentComposer : IFeaturePublicContentComposer
{
    ...
    public FeaturePublicContentDraft Compose(string featureKey, string displayName, IReadOnlyList<FeaturePricing> pricings)
    {
        ...
        var recurringPolicy = RecurringPricingPolicyCodec.ReadFromPricings(initialPricing, renewalPricing);
        pricingBuilder.Append("<p><strong>القيم المالية المعتمدة حالياً:</strong></p><ul>");
        pricingBuilder.Append($"<li>قيمة الإضافة: {BuildPricingAmountLabel(initialPricing)}.</li>");
        ...
        if (recurringPolicy.FreeInitialUnits > 0 || recurringPolicy.FreeRenewalUnits > 0)
        {
            pricingBuilder.Append($"<li>الوحدات المجانية قبل بدء التسعير: {recurringPolicy.FreeInitialUnits:N0}.</li>");
            pricingBuilder.Append($"<li>الوحدات المجانية قبل بدء رسوم التجديد: {recurringPolicy.FreeRenewalUnits:N0}.</li>");
        }
        pricingBuilder.Append("</ul>");
