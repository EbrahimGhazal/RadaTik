        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var billingDueDate = now.Date.AddDays(3);

                    var dueForBillingSoon = await db.NetworkServiceSubscriptions
                        .AsNoTracking()
                        .Where(s =>
                    ...
                    {
                        var networkIds = dueForBillingSoon.Select(s => s.NetworkId).Distinct().ToList();
                        var networks = await db.Networks.AsNoTracking()
                            .Where(n => networkIds.Contains(n.Id))
                            .Select(n => new { n.Id, n.Name, n.ManagerUserId })
                            .ToListAsync(stoppingToken);

                        var mgrByNetworkId = networks
                            .Where(n => !string.IsNullOrWhiteSpace(n.ManagerUserId))
                            .ToDictionary(n => n.Id, n => n.ManagerUserId!, EqualityComparer<int>.Default);

                        foreach (var s in dueForBillingSoon)
                        {
                            if (!mgrByNetworkId.TryGetValue(s.NetworkId, out var managerUserId))
                                continue;

                            var title = "تنبيه: قرب استحقاق رسم الخدمة";
                            var msg =
                                $"الخدمة ({s.FeatureKey}) ستُخصم دورياً خلال 3 أيام. تاريخ الاستحقاق: {s.ExpiresAt:yyyy/MM/dd HH:mm}. تأكد من رصيد المحفظة.";

                            var key = $"SubBillDue3d:{s.Id}:{billingDueDate:yyyyMMdd}";

                            if (!await db.UserNotifications.AsNoTracking().AnyAsync(n => n.Key == key, stoppingToken))
                            {
