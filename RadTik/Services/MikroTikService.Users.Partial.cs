...
public partial class MikroTikService
{
    ...
    public async Task<bool> UpdateUserFromAllUsers(EditMikroTikUserViewModel model)
    {
        ...
        try
        {
            var server = await _context.MikroTikServers.FindAsync(model.MikroTikServerId);
            if (server == null)
            {
            ...
            {
                var profileExists = await CheckProfileExistsInMikroTik(model.MikroTikServerId, model.ProfileName);
                if (!profileExists)
                {
            ...
            var mikrotikResult = await UpdateMikroTikUserProfile(model);
            if (!mikrotikResult)
            {
    ...
    public async Task<bool> UpdateMikroTikUserProfile(EditMikroTikUserViewModel model)
    {
        ...
        try
        {
            ...
            var targetUser = users.FirstOrDefault(u => GetSafeValue(u, "name") == model.UserName);
            ...
            var updateCmd = connection.CreateCommand("/ppp/secret/set");
    ...
    public async Task<bool> CheckProfileExistsInServer(int serverId, string profileName)
    {
        try
        {
            var profiles = await GetProfileNamesFromMikroTik(serverId);
    ...
    public async Task<List<EditMikroTikUserViewModel>> GetAllUsersWithDetails(int serverId)
    {
        ...
        var result = new List<EditMikroTikUserViewModel>();
        ...
        try
        {
            using (var connection = CreateConnectionWithRetry(server))
            {
                var secretCmd = connection.CreateCommand("/ppp/secret/print");
                var secretRows = secretCmd.ExecuteList();

                foreach (var row in secretRows)
                {
                    var username = GetSafeValue(row, "name");
                    var clientInDb = await _context.Clients
                        .FirstOrDefaultAsync(c => c.UserName == username && c.MikroTikServerId == serverId);

                    var userViewModel = new EditMikroTikUserViewModel
                    {
                        UserName = username,
    ...
    public async Task<bool> CheckUserExists(string username, int serverId)
    {
        ...
        try
        {
            using (var connection = CreateConnectionWithRetry(server))
            {
                var checkCmd = connection.CreateCommand("/ppp/secret/print");
                var allUsers = checkCmd.ExecuteList();
    ...
    public async Task<bool> AddPPPoEUser(Client client)
    {
        ...
        var server = await _context.MikroTikServers.FindAsync(client.MikroTikServerId);
        if (server == null)
        {
            throw new InvalidOperationException("الخادم غير موجود");
        }
        ...
        try
        {
            ...
            var checkCmd = connection.CreateCommand("/ppp/secret/print");
            var allUsers = checkCmd.ExecuteList();
            var existingUser = allUsers.FirstOrDefault(u => GetSafeValue(u, "name") == client.UserName);
            ...
            var allProfiles = profileCmd.ExecuteList();
            var profileExists = allProfiles.Any(p => GetSafeValue(p, "name") == client.ProfileName);
            ...
            try
            {
                var addCmd = connection.CreateCommand("/ppp/secret/add");
            ...
            var verifyUser = allUsers.FirstOrDefault(u => GetSafeValue(u, "name") == client.UserName);
    ...
    public async Task<bool> UpdatePPPoEUser(Client client)
    {
        ...
        try
        {
            using (var connection = CreateConnectionWithRetry(server))
            {
                ...
                var targetUser = allUsers.FirstOrDefault(u => GetSafeValue(u, "name") == client.UserName);
    ...
    public async Task<bool> UpdatePPPoEUserWithOriginalUsername(Client client, string originalUsername)
    {
        ...
        try
        {
            using (var connection = CreateConnectionWithRetry(server))
            {
                var findCmd = connection.CreateCommand("/ppp/secret/print");
                var allUsers = findCmd.ExecuteList();
                var targetUser = allUsers.FirstOrDefault(u => GetSafeValue(u, "name") == originalUsername);
                ...
                var newUserName = client.UserName ?? string.Empty;
                if (!string.Equals(originalUsername, newUserName, StringComparison.Ordinal))
                {
                    var duplicate = allUsers.FirstOrDefault(u => GetSafeValue(u, "name") == newUserName);
                    if (duplicate != null)
                    {
    ...
    public async Task<bool> DeletePPPoEUser(string username, int serverId)
    {
        ...
        try
        {
            using (var connection = CreateConnectionWithRetry(server))
            {
                ...
                var deleteCmd = connection.CreateCommand("/ppp/secret/remove");
    ...
    public async Task<List<Client>> GetActivePPPoEUsers(int serverId)
    {
        ...
        try
        {
            using (var connection = CreateConnectionWithRetry(server))
            {
                ...
                var activeCmd = connection.CreateCommand("/ppp/active/print");
                var activeRows = activeCmd.ExecuteList();
    ...
    public async Task<List<Client>> GetAllPPPoEUsers(int serverId)
    {
        ...
        try
        {
            var result = await ExecuteWithRetry(server, connection =>
            {
                var secretCmd = connection.CreateCommand("/ppp/secret/print");
                var secretRows = secretCmd.ExecuteList();
    ...
    public async Task<bool> DisconnectActiveUser(int serverId, string username)
    {
        ...
        try
        {
            using (var connection = CreateConnectionWithRetry(server))
            {
                ...
                var findCmd = connection.CreateCommand("/ppp/active/print");
                var allSessions = findCmd.ExecuteList();
                var targetSession = allSessions.FirstOrDefault(s => GetSafeValue(s, "name") == username);
                ...
                var sessionId = GetSafeValue(targetSession, ".id");

                var disconnectCmd = connection.CreateCommand("/ppp/active/remove");
    ...
    public async Task<bool> DisablePPPoEUser(int serverId, string username)
    {
        ...
        try
        {
            using (var connection = CreateConnectionWithRetry(server))
            {
                ...
                var disableCmd = connection.CreateCommand("/ppp/secret/set");
    ...
    public async Task<bool> EnablePPPoEUser(int serverId, string username)
    {
        ...
        try
        {
            using (var connection = CreateConnectionWithRetry(server))
            {
                ...
                var enableCmd = connection.CreateCommand("/ppp/secret/set");
    ...
    public async Task<ExpiredAccountsResult> CheckAndDisableExpiredAccounts()
    {
        ...
        var result = new ExpiredAccountsResult
        {
        ...
        try
        {
            var clientsWithExpiration = await _context.Clients
            ...
            foreach (var client in clientsWithExpiration)
            {
                try
                {
                    ...
                    var expirationDate = client.AccountExpirationDate ?? DateTime.Now;
