...
namespace RadTik.Areas.SystemAdmin.Controllers
{
    ...
    public class PaymentMethodsController : Controller
    {
        ...
        public async Task<IActionResult> Create(string name, int displayOrder = 0, bool isActive = true, bool isCash = false)
        {
            ...
            try
            {
                var exists = await _context.PaymentMethods.AnyAsync(m => m.Name == name);
                if (exists)
                {
                ...
                TempData["Success"] = "تمت إضافة طريقة الدفع.";
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Update(int id, string name, int displayOrder = 0, bool isActive = true, bool isCash = false)
        {
            ...
            try
            {
                var row = await _context.PaymentMethods.FindAsync(id);
                if (row == null)
                {
                    return NotFound();
                }

                var exists = await _context.PaymentMethods.AnyAsync(m => m.Id != id && m.Name == name);
                if (exists)
                {
                ...
                TempData["Success"] = "تم تحديث طريقة الدفع.";
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                await _context.SaveChangesAsync();
                TempData["Success"] = "تم حذف طريقة الدفع.";
            }
            catch (Exception ex)
            {
