...
public class NotificationsController : Controller
{
    ...
    public async Task<IActionResult> Index(bool unreadOnly = true)
    {
        ...
        var q = _context.UserNotifications
            .AsNoTracking()
    ...
    public async Task<IActionResult> MarkRead(int id, string? returnUrl = null)
    {
        ...
        var row = await _context.UserNotifications.FirstOrDefaultAsync(n => n.Id == id && n.UserId == user.Id);
        if (row == null)
            return NotFound();
    ...
    public async Task<IActionResult> Open(int id)
    {
        ...
        var targetUrl = await ResolveNotificationTargetUrlAsync(row);
    ...
    private async Task<string?> ResolveNotificationTargetUrlAsync(UserNotification notification)
    {
        ...
        {
            var parts = (notification.Key ?? string.Empty).Split(':', StringSplitOptions.RemoveEmptyEntries);
            if (parts.Length >= 2 && int.TryParse(parts[1], out var sectorId))
            {
                var requestId = await _context.NetworkServiceRequests
                    .AsNoTracking()
                    .Where(r =>
        ...
        {
            var parts = (notification.Key ?? string.Empty).Split(':', StringSplitOptions.RemoveEmptyEntries);
            if (parts.Length >= 3 && int.TryParse(parts[2], out var requestId))
            {
        ...
        switch (notification.Type)
        {
            ...
            case NotificationType.SpeedChangeRequestSubmitted:
            {
                ...
                {
                    var parts = (notification.Key ?? string.Empty).Split(':', StringSplitOptions.RemoveEmptyEntries);
                    if (parts.Length >= 2 && int.TryParse(parts[1], out var parsedRequestId))
                    {
