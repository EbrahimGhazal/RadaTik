...
public class CustomServicesController : Controller
{
    ...
    public async Task<IActionResult> Index(string serviceKey)
    {
        ...
        var service = await _context.SystemServices.AsNoTracking().FirstOrDefaultAsync(s => s.Key == serviceKey && s.IsActive);
        if (service == null)
            return NotFound();
        ...
        var items = await _context.CustomServiceItems
            .AsNoTracking()
    ...
    {
        var item = await _context.CustomServiceItems.FindAsync(id);
        if (item == null)
            return NotFound();

        var service = await _context.SystemServices.AsNoTracking().FirstOrDefaultAsync(s => s.Key == item.ServiceKey && s.IsActive);
        if (service == null)
            return NotFound();
    ...
    {
        var existing = await _context.CustomServiceItems.FindAsync(model.Id);
        if (existing == null)
            return NotFound();

        var service = await _context.SystemServices.AsNoTracking().FirstOrDefaultAsync(s => s.Key == existing.ServiceKey && s.IsActive);
        if (service == null)
            return NotFound();
