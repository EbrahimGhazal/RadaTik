...
/**
 * RadTik logomark: network ring, routing strokes, access glow, geometric R silhouette.
...
export function Logo({ className, showWordmark = true, compact = false }: LogoProps) {
  const glowFilterId = `radtik-glow-${useId().replace(/:/g, '')}`
    ...
    <div className={cn('flex items-center gap-2', className)}>
      ...
      {showWordmark ? (
        ...
        >
          ...
          >
            R
            <span className={cn(isDark ? 'text-[#FB923C]' : 'text-[#06B6D4]')}>ad</span>
            T
